...
namespace KrakenClient
{
    ...
    public class KrakenClient : IDisposable
    {
        ...
        private JsonObject QueryPrivate(string a_sMethod, string props = null)
        {
            ...
            //webRequest.Headers.Add("API-Sign", _secret);
        ...
        */
        ...
        ///
        /// </summary>
        /// <param name="pair"> pair is the currency pair for which you wish to receive the data</param>
        /// <param name="interval"></param>
        /// <param name="since"></param>
        /// <returns></returns>
        public JsonObject GetOHLCData(string pair, int interval = 1, long? since = null)
        {
            string reqs = string.Format("pair={0}", pair);
            ...
            return QueryPublic("OHLC", reqs) as JsonObject;
        }
   ...
   /// </returns>
       ...
       /// </returns>
            ...
            Dictionary<string, string> close = null)
        {
            ...
            // START 
            //Add by VL to solve comma issue in converting decimal to string
            NumberFormatInfo NumberProvider = new NumberFormatInfo();
            NumberProvider.CurrencyDecimalSeparator = ".";
            // END

            string reqs = string.Format("&pair={0}&type={1}&ordertype={2}&volume={3}&leverage={4}", pair, type, ordertype, volume.ToString(NumberProvider), leverage);
            if (price.HasValue)
                reqs += string.Format("&price={0}", price.Value.ToString(NumberProvider));
            if (price2.HasValue)
                reqs += string.Format("&price2={0}", price2.Value.ToString(NumberProvider));
